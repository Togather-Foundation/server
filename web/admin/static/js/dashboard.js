...
(function() {
    ...
    function setupSubmissionsButtons() {
        // Copy URL button — points to actionable (pending) submissions only
        document.getElementById('submissions-copy-url-btn')?.addEventListener('click', () => {
            const url = window.location.origin + '/api/v1/admin/scraper/submissions?status=pending';
            copyToClipboard(url);
        });

        // Copy JSON button — fetch pending_validation + pending in parallel, merge results
        document.getElementById('submissions-copy-json-btn')?.addEventListener('click', async () => {
            ...
            try {
                const [pv, p] = await Promise.all([
                    API.scraper.listSubmissions({ status: 'pending_validation', limit: 200 }),
                    API.scraper.listSubmissions({ status: 'pending', limit: 200 }),
                ]);
                const merged = {
                    submissions: [...(pv.submissions || []), ...(p.submissions || [])],
                    total: (pv.total || 0) + (p.total || 0),
                };
