// Package rank holds the canonical primary-election ranking for entity
// identifiers. It is dependency-free so that both the identity store
// (internal/identity) and the storage merge path (internal/storage/postgres) can
// share a single ranking implementation instead of duplicating the order in SQL.
package rank
...
// Outranks reports whether a is strictly higher-ranked than b under the canonical
// order. It is used by the merge dedupe to decide which of two colliding rows
// carries the surviving attributes.
func Outranks(a, b IdentifierObservation) bool {
	return less(b, a)
}
