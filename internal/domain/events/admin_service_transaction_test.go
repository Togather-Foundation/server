...
func TestAddOccurrenceFromReviewNearDup_CommitOnSuccess(t *testing.T) {
	...
	service := NewAdminService(repo, false, "America/Toronto", config.ValidationConfig{}, "https://toronto.togather.foundation", zerolog.Nop())
...
func TestAddOccurrenceFromReview_TombstoneUsesCanonicalURIs(t *testing.T) {
	...
	service := NewAdminService(repo, false, "America/Toronto", config.ValidationConfig{MaxEventNameLength: 500}, baseURL, zerolog.Nop())
...
func TestAddOccurrenceFromReviewNearDup_TombstoneUsesCanonicalURIs(t *testing.T) {
	...
	service := NewAdminService(repo, false, "America/Toronto", config.ValidationConfig{}, baseURL, zerolog.Nop())
	_, _, err := service.AddOccurrenceFromReviewNearDup(ctx, 1, "admin")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestNewAdminService_PanicsOnEmptyBaseURL(t *testing.T) {
	...
	NewAdminService(&mockTransactionalRepo{}, false, "America/Toronto", config.ValidationConfig{}, "", zerolog.Nop())
}
