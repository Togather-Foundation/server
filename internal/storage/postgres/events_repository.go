...
type reviewQueueRowFields interface {
	...
	GetDuplicateOfEventUlid() pgtype.Text
}
...
func (r FindReviewByDedupRow) GetDuplicateOfEventUlid() pgtype.Text  { return r.DuplicateOfEventUlid }
...
func (r ListReviewQueueRow) GetDuplicateOfEventUlid() pgtype.Text  { return r.DuplicateOfEventUlid }
...
func (r GetReviewQueueEntryRow) GetDuplicateOfEventUlid() pgtype.Text  { return r.DuplicateOfEventUlid }
...
func convertReviewQueueRowGeneric(row reviewQueueRowFields) *events.ReviewQueueEntry {
	...
	if duplicateOfULID := row.GetDuplicateOfEventUlid(); duplicateOfULID.Valid {
		entry.DuplicateOfEventULID = &duplicateOfULID.String
	}
...
func (r *EventRepository) UpdateReviewQueueEntry(ctx context.Context, id int, params events.ReviewQueueUpdateParams) (*events.ReviewQueueEntry, error) {
	...
	} else if params.DuplicateOfEventID != nil {
		var dupUUID pgtype.UUID
		if err := dupUUID.Scan(*params.DuplicateOfEventID); err != nil {
...
func (r *EventRepository) StripRetiredDupWarnings(ctx context.Context, reviewID int32, retireULIDs []string) (bool, error) {
	queries := Queries{db: r.queryer()}
