...
func (r *ChangeFeedRepository) ListEventChanges(ctx context.Context, arg federation.ListEventChangesParams) ([]federation.ListEventChangesRow, error) {
	...
	for i, row := range rows {
		result[i] = federation.ListEventChangesRow{
			...
			OccurrenceStartTime:  row.OccurrenceStartTime,
			OccurrenceEndTime:    row.OccurrenceEndTime,
			VenueName:            row.VenueName,
			VenueStreetAddress:   row.VenueStreetAddress,
			VenueAddressLocality: row.VenueAddressLocality,
			VenueAddressRegion:   row.VenueAddressRegion,
			VenueAddressCountry:  row.VenueAddressCountry,
		}
	}
