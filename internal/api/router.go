...
func NewRouter(cfg config.Config, logger zerolog.Logger, pool *pgxpool.Pool, version, gitCommit, buildDate string, shutdownCtx ...context.Context) *RouterWithClient {
	...
	eventsService := events.NewService(repo.Events()).WithGeographicBoundaryConfig(cfg.GeographicBoundary)
	...
	adminAuthHandler := handlers.NewAdminAuthHandler(queries, jwtManager, auditLogger, cfg.Environment, adminTemplates)
	...
	handler = middleware.OpenAPILink(cfg.Server.BaseURL)(handler)
