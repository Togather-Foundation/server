...
Rate limiting is implemented in the Togather application layer. To add additional reverse proxy rate limiting, consider using a Caddy rate limit plugin or nginx.
...
Key differences from the nginx setup in `deploy/docker/nginx.conf`:

| Feature | Nginx | Caddy |
|---------|-------|-------|
| SSL Setup | Manual (certbot) | Automatic |
| Config Reload | `nginx -s reload` | `systemctl reload caddy` |
| Rate Limiting | Built-in | App-level or plugin |
| Blue-Green Switch | Edit upstream {} | Edit reverse_proxy |
...
3. Install and configure Caddy (follow this guide)
4. Update firewall rules
5. Start Caddy: `sudo systemctl start caddy`
6. Verify everything works
7. Optional: Remove nginx: `sudo apt remove nginx`
