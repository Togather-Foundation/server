...
| `RRULE expanded to zero occurrences (UID: <uid>) — all outside the <N>-day horizon or in the past` | All RRULE-expanded occurrences fall outside the `ICS_HORIZON_DAYS` window or have already ended | Increase `ICS_HORIZON_DAYS` env var (default: 90) to expand the window, or check that the feed's DTSTART is not in the distant past. The event is skipped — no occurrences are ingested. |
